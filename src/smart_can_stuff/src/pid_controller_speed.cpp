...
: Node("speed_pid_node", options), // ROS 2 düğümünün adını ve seçeneklerini ayarla
  ...
  m_target_data_since_lastcb(0)
{
    ...
    declare_parameter<std::string>("set_value_topic", "/beemobs/speed_target_value");
    declare_parameter<std::string>("feedback_value_topic", "/beemobs/Fbvehiclespeed");
    declare_parameter<std::string>("output_topic", "/beemobs/Rcthrtdata");
    ...
    m_feedback_speed_value_subscriber = this->create_subscription<smart_can_msgs::msg::Fbvehiclespeed>(
        m_feedback_value_topic, 10, std::bind(&PIDController::feedbackspeedValueCallback, this, _1));

    // Çıktı mesajlarını yaymak için yayımcı oluştur
    m_output_speed_publisher = this->create_publisher<smart_can_msgs::msg::Rcthrtdata>(
        m_output_topic, 10);
}
...
void PIDController::feedbackspeedValueCallback(const smart_can_msgs::msg::Fbvehiclespeed::SharedPtr t_feedback_speed_value)
{
    // Gelen feedback değerini güncelle
    m_feedback_value = t_feedback_speed_value->fb_vehiclespeed_kmh;
    if (m_target_data_since_lastcb < m_timeout_threshold)
    {
        ...
        if (m_timeout_threshold - m_target_data_since_lastcb == 1)
        {
            ...
            smart_can_msgs::msg::Rcthrtdata msg;
            msg.rc_thrt_pedal_position = 0;
            msg.rc_thrt_pedal_press = 1;
            m_output_speed_publisher->publish(msg);
        }
    }
...
void PIDController::sendToTopic()
{
    ...
    if (m_feedback_value != m_set_value)
    {
        if (m_output > 0)
        {
            msg.rc_thrt_pedal_position = m_output;
    ...
    m_output_speed_publisher->publish(msg); // Mesajı yay
}
